...
class Api::V1::ProductsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should destroy product' do
    assert_difference('Product.count', -1) do
      # passing @product will construct the url
      # so rails knows to which method it shoud
      # go to. The id will be in the url
  ...
  test 'should forbid destroy product' do
    assert_no_difference('Product.count') do
      delete api_v1_product_url(@product),
        headers: { Authorization: JsonWebToken.encode(user_id: products(:two).user_id) },
        as: :json 
    end
    assert_response :forbidden
  end
